...
const AddCourse = () => {
  ...
  const addLecture = () => {
    const embeddedUrl = convertToEmbedUrl(lectureDetails.pptUrl);
    setChapters(
      chapters.map((chapter) => {
        if (chapter.chapterId === currentChapterId) {
          const newLecture = {
            ...lectureDetails,
            lectureUrl: embeddedUrl,
    ...
    try {
      ...
      const transformedChapters = chapters.map((chapter) => ({
        ...
        chapterContent: chapter.chapterContent.map((lecture) => ({
          ...lecture,
          lectureUrl: convertToEmbedUrl(lecture.pptUrl),
        })),
      }));
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div className='flex flex-col gap-1'>
          <p>Discount %</p>
          <input type="number" value={discount} onChange={e => setDiscount(e.target.value)} required min={0} max={100}
            className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' />
        </div>

        {/* Chapter and Lecture section */}
        <div>
              ...
              <div className="flex justify-between items-center p-4 border-b">
                <div className="flex items-center">
                  <img className={`mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`}
                    onClick={() => handleChapter('toggle', chapter.chapterId)} src={assets.dropdown_icon} width={14} alt="" />
                  ...
                  {chapter.chapterContent.map((lecture, lectureIndex) => (
                    <div key={lectureIndex} className="flex justify-between items-center mb-2">
                      <span>
                        ...
                        <a href={lecture.lectureUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 ml-1">View PPT</a> - 
          ...
          {showPopup && (
            <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
              <div className="bg-white text-gray-700 p-4 rounded relative w-full max-w-80">
                ...
                <div className="mb-2">
                  <p>PPT Canva/Embed URL</p>
                  <input type="text" className="mt-1 block w-full border rounded py-1 px-2"
                    value={lectureDetails.pptUrl}
                    onChange={(e) => setLectureDetails({ ...lectureDetails, pptUrl: e.target.value })} />
                </div>
